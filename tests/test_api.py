...
#     url = reverse('usercheckskills-smart-generate')  # Проверьте правильность этого URL
#     response = client.post(url, {'course_id': course.id}, format='json')

#     assert response.status_code == status.HTTP_200_OK  # Проверьте правильный статус-код для успешного ответа
...
#     url = reverse('userticket-generate-random-ticket')  # Проверьте правильность этого URL
#     response = client.post(url, {'course_id': course.id}, format='json')
