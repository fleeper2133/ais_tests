            ...
            <div class="content__gap">
                ...
                <div class="container container__courses">
                    ...
                    <div v-if="paginatedItems.length > 0" class="courses">
                        <div v-for="(course, index) in currentCourses" :key="course.id" class="course">
...
const courses = computed(() => {
    return {
        ...
        'New': paginatedItems.value.filter(c2 => aisStore.startedCourses.find(c => c.course === c2.id))
    }
})
...
const totalItems = computed(() => {
    if (showCourseStatus.value === 'All') {
        return filterSearch.value.length
    } else if (showCourseStatus.value === 'New') {
        return filterSearch.value.filter(c2 => aisStore.startedCourses.find(c => c.course === c2.id)).length
    }
    return 0
});

const showPagination = computed(() => {
    return totalItems.value > itemsPerPage
});
