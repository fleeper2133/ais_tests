    ...
    <div class="content" @click="closeAllDroppers">
        <div>
            ...
            <div class="container container__header">
                <div class="fs-18 fw-bold">Избранные вопросы</div>
            </div>
            <div class="container">
                <div class="content__items" v-if="!aisStore.isLoading">
                    <div v-if="aisStore.courseQuestions.filter(cq => cq['selected']).length > 0" class="creator">
                        <div class="creator__text">
                            ...
                            <div class="creator__data">
                                <div class="selector__data">
                                    ...
                                    <p class="yellow fs-14 fw-bold">{{ aisStore.courseQuestions.filter(cq => cq['selected']).length }}</p>
                                </div>
                                <div v-if="aisStore.courseQuestions.filter(cq => cq['selected']).length >= 20" class="selector__data">
                                    <p class="white fs-14">Выбрано вопросов:</p>
                                    <p class="yellow fs-14 fw-bold">{{ questionCount }}</p> 
                                </div>
                            </div>
                            <div v-if="aisStore.courseQuestions.filter(cq => cq['selected']).length >= 20" class="dropper">
                                ...
                                <div v-show="questions" class="dropper__list">
                                    ...
                                    <p v-if="aisStore.courseQuestions.filter(cq => cq['selected']).length >= 30" class="dropper__item" @click.stop="showedValue(30)">30 вопросов</p>
                                    <p v-if="aisStore.courseQuestions.filter(cq => cq['selected']).length >= 40" class="dropper__item" @click.stop="showedValue(40)">40 вопросов</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="questions" v-if="filterSearch.length > 0">
                        ...
                        >
                            <div class="question_header">
                                <p class="fs-14 grey-text">ID вопроса: {{ question.id }}</p>
...
const filterSearch = computed(() => {
    const selectedCourseQuestions = aisStore.courseQuestions.filter(cq => cq['selected'] === true)
    ...
    return selectedCourseQuestions.filter((question) => {
        ...
        const questionId = question.id?.toString().toLowerCase()

        return questionText?.includes(inputText.value.toLowerCase()) || questionId?.includes(inputText.value.toLowerCase())
    })
})
...
async function generateCheck() {
    ...
    const selectedQuestionsCount = aisStore.courseQuestions.filter(cq => cq['selected']).length

    const data: GenerateCheck = {
        user_course_id: startedCourse.id
    };

    if (selectedQuestionsCount >= 20) {
        data.question_count = questionCount.value
    }

    await aisStore.generateFavouriteCheck(data)
