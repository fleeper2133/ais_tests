<template>
    <div class="content">
        <div>
            <Header />
            <div class="my-container container__bg link-line">
            ...
            <div>
                <div class="my-container ticket-list">
                    ...
                    <div v-if="!isTestDone" class="question-interaction">
                        <div class="favorites" @click.stop="favoriteQuestion($event, currentQuestion?.id)">
                            <p class="fs-14 fw-bold">В избранное</p>
                            <svg class="star" viewBox="-0.5 0 25 25" :class="currentQuestion?.selected ? 'star--filled' : 'none'">
                        ...
                        <div class="complaint" @click="aisStore.isQuestionComplanePopupVisible = true">
                            <p class="fs-14 fw-bold">Пожаловаться</p>
                            <svg class="complaint__icon" fill="#000000" width="30px" height="30px" viewBox="0 0 24 24"><path d="M12 2C6.486 2 2 6.486 2 12v4.143C2 17.167 2.897 18 4 18h1a1 1 0 0 0 1-1v-5.143a1 1 0 0 0-1-1h-.908C4.648 6.987 7.978 4 12 4s7.352 2.987 7.908 6.857H19a1 1 0 0 0-1 1V18c0 1.103-.897 2-2 2h-2v-1h-4v3h6c2.206 0 4-1.794 4-4 1.103 0 2-.833 2-1.857V12c0-5.514-4.486-10-10-10z"/></svg>
                        </div>
                    </div>
                </div>
                        ...
                        <div v-if="currentQuestionIndex !== null && !isTestDone">
                            <div class="title">
                                <div class="title__header">
                                    ...
                                    <h1 class="fs-14 grey-text" >ID вопроса: {{ currentQuestion['id'] }}</h1>
                                </div>
...
import { computed, onMounted, ref, watch, onUnmounted, onBeforeMount, onBeforeUnmount } from 'vue'
...
const totalSeconds = ref(aisStore.testingDetail['questions'].length * 80) // 2 минуты на каждый вопрос
let interval
...
const getElapsedTime = () => {
    return (aisStore.testingDetail['questions'].length * 80) - totalSeconds.value
}
...
function setQuestionDataToStorage() {
    return aisStore.questionDataForComplane = allQuestions.value[currentQuestionIndex.value]
}

async function favoriteQuestion(event, id){
